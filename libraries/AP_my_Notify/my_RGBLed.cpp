...
my_RGBLed::rgb_source_t my_RGBLed::rgb_source() const
{
...
uint8_t my_RGBLed::get_brightness(void) const
{
...
uint32_t my_RGBLed::get_colour_sequence_obc(void) const
{
...
uint32_t my_RGBLed::get_colour_sequence(void) const
{
    ...
    if (AP_my_Notify::flags.save_trim || AP_my_Notify::flags.esc_calibration) {
    ...
    if (AP_my_Notify::flags.failsafe_radio ||
        AP_my_Notify::flags.failsafe_battery ||
        AP_my_Notify::flags.ekf_bad ||
        AP_my_Notify::flags.gps_glitching ||
        AP_my_Notify::flags.leak_detected) {

        if (AP_my_Notify::flags.leak_detected) {
            // purple if leak detected
            return sequence_failsafe_leak;
        } else if (AP_my_Notify::flags.ekf_bad) {
            // red on if ekf bad
            return sequence_failsafe_ekf;
        } else if (AP_my_Notify::flags.gps_glitching) {
            // blue on gps glitch
    ...
    if (AP_my_Notify::flags.armed) {
        ...
        if (AP_my_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D) {
    ...
    if (!AP_my_Notify::flags.pre_arm_check) {
        return sequence_prearm_failing;
    }
    if (AP_my_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D_DGPS && AP_my_Notify::flags.pre_arm_gps_check) {
        return sequence_disarmed_good_dgps;
    }

    if (AP_my_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D && AP_my_Notify::flags.pre_arm_gps_check) {
...
uint32_t my_RGBLed::get_colour_sequence_traffic_light(void) const
{
    if (AP_my_Notify::flags.initialising) {
...
*/
void my_RGBLed::handle_led_control(const mavlink_message_t &msg)
{
    if (rgb_source() != mavlink) {
