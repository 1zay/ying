...
bool my_Display::init(void)
{
    ...
    FOREACH_I2C(i) {
        ...
        case DISPLAY_SSD1306: {
            _driver = my_Display_SSD1306_I2C::probe(std::move(hal.i2c_mgr->get_device(i, NOTIFY_DISPLAY_I2C_ADDR)));
            break;
        }
        case DISPLAY_SH1106: {
            _driver = my_Display_SH1106_I2C::probe(std::move(hal.i2c_mgr->get_device(i, NOTIFY_DISPLAY_I2C_ADDR)));
            break;
        }
        case DISPLAY_SITL: {
#ifdef WITH_SITL_OSD
            _driver = my_Display_SITL::probe(); // never fails
...
void my_Display::draw_text(uint16_t x, uint16_t y, const char* c)
{
    if (nullptr == c) {
        return;
    }
...
void my_Display::draw_char(uint16_t x, uint16_t y, const char c)
{
    uint8_t line;
...
void my_Display::update_arm(uint8_t r)
{
    if (AP_my_Notify::flags.armed) {
...
void my_Display::update_prearm(uint8_t r)
{
...
void my_Display::update_gps(uint8_t r)
{
    ...
    snprintf(msg, DISPLAY_MESSAGE_SIZE, "GPS:%-5s Sats:%2u", fixname, (unsigned)AP_my_Notify::flags.gps_num_sats) ;
    draw_text(COLUMN(0), ROW(r), msg);
}

void my_Display::update_gps_sats(uint8_t r)
{
    draw_text(COLUMN(0), ROW(r), "Sats:");
    draw_char(COLUMN(8), ROW(r), (AP_my_Notify::flags.gps_num_sats / 10) + '0');
    draw_char(COLUMN(9), ROW(r), (AP_my_Notify::flags.gps_num_sats % 10) + '0');
}

void my_Display::update_ekf(uint8_t r)
{
    if (AP_my_Notify::flags.ekf_bad) {
...
void my_Display::update_battery(uint8_t r)
{
    char msg [DISPLAY_MESSAGE_SIZE];
...
void my_Display::update_mode(uint8_t r)
{
    char msg [DISPLAY_MESSAGE_SIZE];
...
void my_Display::update_text_empty(uint8_t r)
{
    char msg [DISPLAY_MESSAGE_SIZE] = {};
...
void my_Display::update_text(uint8_t r)
{
    char msg [DISPLAY_MESSAGE_SIZE] = {};
