...
my_NCP5623::my_NCP5623(uint8_t bus)
    : my_RGBLed(NCP5623_LED_OFF, NCP5623_LED_BRIGHT, NCP5623_LED_MEDIUM, NCP5623_LED_DIM)
    , _bus(bus)
{
}

bool my_NCP5623::write(uint8_t reg, uint8_t data)
{
    uint8_t msg[1] = { 0x00 };
...
bool my_NCP5623::write_pwm(uint8_t _rgb[3])
{
...
bool my_NCP5623::hw_init(void)
{
    ...
    for (uint8_t i=0; i<ARRAY_SIZE(addrs); i++) {
        ...
        if (ret) {
            _dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&my_NCP5623::_timer, void));
        }
        return true;
    }
...
bool my_NCP5623::hw_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    rgb[0] = red;
    rgb[1] = green;
    rgb[2] = blue;
