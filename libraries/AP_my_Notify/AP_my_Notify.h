...
class AP_my_Notify
{
    ...
    friend class my_Display;           // Display needs access to notify parameters
public:
    AP_my_Notify();

    /* Do not allow copies */
    AP_my_Notify(const AP_my_Notify &other) = delete;
    AP_my_Notify &operator=(const AP_my_Notify&) = delete;

    // get singleton instance
    static AP_my_Notify *get_singleton(void) {
        return _singleton;
    }
...
private:
    ...
    static AP_my_Notify *_singleton;

    void add_backend_helper(my_NotifyDevice *backend);
    ...
    static my_NotifyDevice* _devices[];
