...
const AP_my_ToneAlarm::Tone AP_my_ToneAlarm::_tones[] {
...
bool AP_my_ToneAlarm::init()
{
    ...
    flags.armed = AP_my_Notify::flags.armed;
    flags.failsafe_battery = AP_my_Notify::flags.failsafe_battery;
    flags.pre_arm_check = 1;
    _cont_tone_playing = -1;
    hal.scheduler->register_timer_process(FUNCTOR_BIND(this, &AP_my_ToneAlarm::_timer_task, void));
...
void AP_my_ToneAlarm::play_tone(const uint8_t tone_index)
{
...
void AP_my_ToneAlarm::_timer_task()
{
    WITH_SEMAPHORE(_sem);
    _mml_player.update();
}

void AP_my_ToneAlarm::play_tune(const char *str)
{
    WITH_SEMAPHORE(_sem);
...
void AP_my_ToneAlarm::stop_cont_tone()
{
...
void AP_my_ToneAlarm::check_cont_tone()
{
    uint32_t tnow_ms = AP_HAL::millis();
...
void AP_my_ToneAlarm::update()
{
    ...
    if (AP_my_Notify::flags.powering_off) {
        if (!flags.powering_off) {
            play_tone(AP_NOTIFY_TONE_QUIET_SHUTDOWN);
        }
        flags.powering_off = AP_my_Notify::flags.powering_off;
        return;
    }

    if (AP_my_Notify::flags.compass_cal_running != flags.compass_cal_running) {
        if (AP_my_Notify::flags.compass_cal_running) {
    ...
    flags.compass_cal_running = AP_my_Notify::flags.compass_cal_running;

    if (AP_my_Notify::events.compass_cal_canceled) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEU_FEEDBACK);
        return;
    }

    if (AP_my_Notify::events.initiated_compass_cal) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEU_FEEDBACK);
        return;
    }

    if (AP_my_Notify::events.compass_cal_saved) {
        play_tone(AP_NOTIFY_TONE_QUIET_READY_OR_FINISHED);
        return;
    }

    if (AP_my_Notify::events.compass_cal_failed) {
    ...
    if (AP_my_Notify::flags.armed && (AP_my_Notify::events.autotune_complete || AP_my_Notify::events.mission_complete)) {
        play_tone(AP_NOTIFY_TONE_LOUD_READY_OR_FINISHED);
    }

    //notify the user when autotune fails
    if (AP_my_Notify::flags.armed && (AP_my_Notify::events.autotune_failed)) {
        play_tone(AP_NOTIFY_TONE_LOUD_NEG_FEEDBACK);
    }

    // notify the user when a waypoint completes
    if (AP_my_Notify::events.waypoint_complete) {
        play_tone(AP_NOTIFY_TONE_LOUD_WP_COMPLETE);
    }

    // notify the user when their mode change was successful
    if (AP_my_Notify::events.user_mode_change) {
        if (AP_my_Notify::flags.armed) {
    ...
    if (AP_my_Notify::events.user_mode_change_failed) {
        if (AP_my_Notify::flags.armed) {
            play_tone(AP_NOTIFY_TONE_LOUD_NEG_FEEDBACK);
        } else {
            play_tone(AP_NOTIFY_TONE_QUIET_NEG_FEEDBACK);
    ...
    if (AP_my_Notify::events.failsafe_mode_change) {
        play_tone(AP_NOTIFY_TONE_LOUD_ATTENTION_NEEDED);
    }

    // notify the user when arming fails
    if (AP_my_Notify::events.arming_failed) {
        play_tone(AP_NOTIFY_TONE_QUIET_NEG_FEEDBACK);
    }

    // notify the user when RC contact is lost
    if (flags.failsafe_radio != AP_my_Notify::flags.failsafe_radio) {
        flags.failsafe_radio = AP_my_Notify::flags.failsafe_radio;
    ...
    if (flags.pre_arm_check != AP_my_Notify::flags.pre_arm_check) {
        flags.pre_arm_check = AP_my_Notify::flags.pre_arm_check;
    ...
    if (flags.armed != AP_my_Notify::flags.armed) {
        flags.armed = AP_my_Notify::flags.armed;
        if (flags.armed) {
            // arming tune
    ...
    if (flags.failsafe_battery != AP_my_Notify::flags.failsafe_battery) {
        flags.failsafe_battery = AP_my_Notify::flags.failsafe_battery;
    ...
    if (flags.parachute_release != AP_my_Notify::flags.parachute_release) {
        flags.parachute_release = AP_my_Notify::flags.parachute_release;
    ...
    // lost vehicle tone
    if (flags.vehicle_lost != AP_my_Notify::flags.vehicle_lost) {
        flags.vehicle_lost = AP_my_Notify::flags.vehicle_lost;
    ...
    if (flags.waiting_for_throw != AP_my_Notify::flags.waiting_for_throw) {
        flags.waiting_for_throw = AP_my_Notify::flags.waiting_for_throw;
    ...
    if (flags.leak_detected != AP_my_Notify::flags.leak_detected) {
        flags.leak_detected = AP_my_Notify::flags.leak_detected;
        if (flags.leak_detected) {
    ...
    if (AP_my_Notify::events.tune_started) {
        play_tone(AP_NOTIFY_TONE_TUNING_START);
        AP_my_Notify::events.tune_started = 0;
    }
    if (AP_my_Notify::events.tune_next) {
        // signify which parameter in the set is starting
        play_tone(AP_NOTIFY_TONE_LOUD_1 + (AP_my_Notify::events.tune_next-1));
        AP_my_Notify::events.tune_next = 0;
    }
    if (AP_my_Notify::events.tune_save) {
        play_tone(AP_NOTIFY_TONE_TUNING_SAVE);
        AP_my_Notify::events.tune_save = 0;
    }
    if (AP_my_Notify::events.tune_error) {
        play_tone(AP_NOTIFY_TONE_TUNING_ERROR);
        AP_my_Notify::events.tune_error = 0;
    }
}
 ...
 */
void AP_my_ToneAlarm::handle_play_tune(const mavlink_message_t &msg)
{
