...
#include "my_Display_SITL.h"

#include <AP_HAL/AP_HAL.h>
#include <AP_my_Notify/AP_my_Notify.h>

#include <stdio.h>
#include <unistd.h>

// constructor
my_Display_SITL::my_Display_SITL()
{
}
...
my_Display_SITL *my_Display_SITL::probe()
{
    my_Display_SITL *driver = new my_Display_SITL();
...
void my_Display_SITL::update_thread(void)
{
    {
        WITH_SEMAPHORE(AP::my_Notify().sf_window_mutex);
...
void *my_Display_SITL::update_thread_start(void *obj)
{
    ((my_Display_SITL *)obj)->update_thread();
    return nullptr;
}

bool my_Display_SITL::hw_init()
{
...
void my_Display_SITL::hw_update()
{
    _need_hw_update = true;
}

void my_Display_SITL::set_pixel(uint16_t x, uint16_t y)
{
    // check x, y range
    if ((x >= COLUMNS) || (y >= ROWS)) {
        return;
...
void my_Display_SITL::clear_pixel(uint16_t x, uint16_t y)
{
    // check x, y range
    if ((x >= COLUMNS) || (y >= ROWS)) {
        return;
...
void my_Display_SITL::clear_screen()
{
    WITH_SEMAPHORE(mutex);
