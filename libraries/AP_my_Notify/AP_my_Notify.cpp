...
#include "my_PixRacerLED.h"
#include "my_Buzzer.h"
#include "my_Display.h"
#include "my_ExternalLED.h"
#include "my_PCA9685LED_I2C.h"
#include "my_NeoPixel.h"
#include "my_NCP5623.h"
#include "my_OreoLED_I2C.h"
#include "my_RCOutputRGBLed.h"
#include "my_ToneAlarm.h"
#include "my_ToshibaLED_I2C.h"
#include "my_VRBoard_LED.h"
#include "my_DiscreteRGBLed.h"
#include "my_DiscoLED.h"
#include "my_Led_Sysfs.h"
#include "my_UAVCAN_RGB_LED.h"
#include "my_SITL_SFML_LED.h"
#include <stdio.h>
#include "AP_my_BoardLED2.h"

extern const AP_HAL::HAL& hal;

AP_my_Notify *AP_my_Notify::_singleton;
...
const AP_Param::GroupInfo AP_my_Notify::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_BRIGHT", 0, AP_my_Notify, _rgb_led_brightness, RGB_LED_HIGH),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_ENABLE", 1, AP_my_Notify, _buzzer_enable, BUZZER_ENABLE_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_OVERRIDE", 2, AP_my_Notify, _rgb_led_override, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("DISPLAY_TYPE", 3, AP_my_Notify, _display_type, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("OREO_THEME", 4, AP_my_Notify, _oreo_theme, 0),
#endif
    ...
    // @Values: 0:Disabled
    // @User: Advanced
    AP_GROUPINFO("BUZZ_PIN", 5, AP_my_Notify, _buzzer_pin, 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_TYPES", 6, AP_my_Notify, _led_type, BUILD_DEFAULT_LED_TYPE),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_ON_LVL", 7, AP_my_Notify, _buzzer_level, 1),
#endif
    ...
    // @Range: 0 100
    // @Units: %
    AP_GROUPINFO("BUZZ_VOLUME", 8, AP_my_Notify, _buzzer_volume, 100),
    
    AP_GROUPEND
};

// Default constructor
AP_my_Notify::AP_my_Notify()
{
...
struct AP_my_Notify::notify_flags_and_values_type AP_my_Notify::flags;
struct AP_my_Notify::notify_events_type AP_my_Notify::events;

my_NotifyDevice *AP_my_Notify::_devices[CONFIG_NOTIFY_DEVICES_MAX];
uint8_t AP_my_Notify::_num_devices;

void AP_my_Notify::add_backend_helper(my_NotifyDevice *backend)
{
...
void AP_my_Notify::add_backends(void)
{
    if (_num_devices != 0) {
        return;
    }
    ...
    for (uint32_t i = 1; i < Notify_LED_MAX; i = i << 1) {
        switch(_led_type & i) {
            ...
            case Notify_LED_ToshibaLED_I2C_Internal:
                ADD_BACKEND(new my_ToshibaLED_I2C(TOSHIBA_LED_I2C_BUS_INTERNAL));
                break;
            case Notify_LED_ToshibaLED_I2C_External:
                ADD_BACKEND(new my_ToshibaLED_I2C(TOSHIBA_LED_I2C_BUS_EXTERNAL));
                break;
#if !HAL_MINIMIZE_FEATURES
            case Notify_LED_NCP5623_I2C_External:
                FOREACH_I2C_EXTERNAL(b) {
                    ADD_BACKEND(new my_NCP5623(b));
                }
                break;
            case Notify_LED_NCP5623_I2C_Internal:
                ADD_BACKEND(new my_NCP5623(TOSHIBA_LED_I2C_BUS_INTERNAL));
                break;
#endif
            case Notify_LED_PCA9685LED_I2C_External:
                ADD_BACKEND(new my_PCA9685LED_I2C());
                break;
            case Notify_LED_NeoPixel:
                ADD_BACKEND(new my_NeoPixel());
                break;
            case Notify_LED_OreoLED:
#if !HAL_MINIMIZE_FEATURES
                if (_oreo_theme) {
                    ADD_BACKEND(new my_OreoLED_I2C(0, _oreo_theme));
                }
#endif
                break;
            case Notify_LED_UAVCAN:
#if HAL_WITH_UAVCAN
                ADD_BACKEND(new my_UAVCAN_RGB_LED(0));
    ...
    ADD_BACKEND(new my_Display());

// ChibiOS noise makers
#if CONFIG_HAL_BOARD == HAL_BOARD_CHIBIOS
    ADD_BACKEND(new my_Buzzer());
#ifdef HAL_PWM_ALARM
    ADD_BACKEND(new AP_my_ToneAlarm());
#endif
...
// initialisation
void AP_my_Notify::init(void)
{
    // clear all flags and events
    memset(&AP_my_Notify::flags, 0, sizeof(AP_my_Notify::flags));
    memset(&AP_my_Notify::events, 0, sizeof(AP_my_Notify::events));
...
void AP_my_Notify::handle_led_control(const mavlink_message_t &msg)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_my_Notify::handle_play_tune(const mavlink_message_t &msg)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_my_Notify::play_tune(const char *tune)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_my_Notify::set_flight_mode_str(const char *str)
{
    strncpy(_flight_mode_str, str, 4);
    _flight_mode_str[sizeof(_flight_mode_str)-1] = 0;
}

void AP_my_Notify::send_text(const char *str)
{
...
AP_my_Notify &my_Notify()
{
    return *AP_my_Notify::get_singleton();
}
