...
bool AP_my_BoardLED2::init(void)
{
 ...
 */
void AP_my_BoardLED2::update(void)
{
    _counter++;
    ...
    if(AP_my_Notify::flags.compass_cal_running){ // compass calibration
        switch(save_trim_counter) {
        case 0:
    ...
    if(AP_my_Notify::events.autotune_complete){
        switch(save_trim_counter) {
        case 0:
    ...
    if(AP_my_Notify::events.autotune_failed){
        switch(save_trim_counter) {
        case 0:
    ...
    if(!led_a_used) {
        if (AP_my_Notify::flags.armed) {
            if(AP_my_Notify::flags.failsafe_battery){//   blink slowly (around 2Hz)
                if ((counter2 & 0x7) == 0) {
                    hal.gpio->toggle(HAL_GPIO_A_LED_PIN);
                }
            }else if(AP_my_Notify::flags.failsafe_radio){//   blink fast (around 4Hz)
                if ((counter2 & 0x3) == 0) {
                    hal.gpio->toggle(HAL_GPIO_A_LED_PIN);
                }
            } else {
    ...
    // gps light
    switch (AP_my_Notify::flags.gps_status) {
        case 0:
        case 1:
        ...
        default: // show number of sats 
            ...
            uint16_t sats = AP_my_Notify::flags.gps_num_sats;
...
#else
bool AP_my_BoardLED2::init(void) {return true;}
void AP_my_BoardLED2::update(void) {return;}
#endif
