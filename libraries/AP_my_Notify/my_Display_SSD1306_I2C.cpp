 ...
 */
#include "my_Display_SSD1306_I2C.h"
...
// constructor
my_Display_SSD1306_I2C::my_Display_SSD1306_I2C(AP_HAL::OwnPtr<AP_HAL::Device> dev) :
    _dev(std::move(dev))
{
}

my_Display_SSD1306_I2C::~my_Display_SSD1306_I2C()
{
}


my_Display_SSD1306_I2C *my_Display_SSD1306_I2C::probe(AP_HAL::OwnPtr<AP_HAL::Device> dev)
{
    my_Display_SSD1306_I2C *driver = new my_Display_SSD1306_I2C(std::move(dev));
...
bool my_Display_SSD1306_I2C::hw_init()
{
    struct PACKED {
        uint8_t reg;
        uint8_t seq[31];
    ...
    if (success) {
        ...
        _dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&my_Display_SSD1306_I2C::_timer, void));
    }

    return success;
}

void my_Display_SSD1306_I2C::hw_update()
{
    _need_hw_update = true;
}

void my_Display_SSD1306_I2C::_timer()
{
    if (!_need_hw_update) {
        return;
    }
...
void my_Display_SSD1306_I2C::set_pixel(uint16_t x, uint16_t y)
{
    // check x, y range
...
void my_Display_SSD1306_I2C::clear_pixel(uint16_t x, uint16_t y)
{
    // check x, y range
    if ((x >= SSD1306_COLUMNS) || (y >= SSD1306_ROWS)) {
        return;
...
void my_Display_SSD1306_I2C::clear_screen()
{
     memset(_displaybuffer, 0, SSD1306_COLUMNS * SSD1306_ROWS_PER_PAGE);
}
