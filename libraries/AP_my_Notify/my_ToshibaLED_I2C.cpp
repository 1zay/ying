...
my_ToshibaLED_I2C::my_ToshibaLED_I2C(uint8_t bus)
    : my_RGBLed(TOSHIBA_LED_OFF, TOSHIBA_LED_BRIGHT, TOSHIBA_LED_MEDIUM, TOSHIBA_LED_DIM)
    , _bus(bus)
{
}

bool my_ToshibaLED_I2C::hw_init(void)
{
    ...
    if (ret) {
        _dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&my_ToshibaLED_I2C::_timer, void));
    }
...
bool my_ToshibaLED_I2C::hw_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    rgb = {red, green, blue};
    _need_update = true;
    return true;
...
void my_ToshibaLED_I2C::_timer(void)
{
    if (!_need_update) {
        return;
    }
