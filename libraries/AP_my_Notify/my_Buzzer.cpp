...
bool my_Buzzer::init()
{
    ...
    _flags.armed = AP_my_Notify::flags.armed;
    _flags.failsafe_battery = AP_my_Notify::flags.failsafe_battery;
    return true;
}

// update - updates led according to timed_updated.  Should be called at 50Hz
void my_Buzzer::update()
{
    update_pattern_to_play();
    update_playing_pattern();
}

void my_Buzzer::update_pattern_to_play()
{
    ...
    if (_flags.armed != AP_my_Notify::flags.armed) {
        _flags.armed = AP_my_Notify::flags.armed;
        if (_flags.armed) {
    ...
    // check ekf bad
    if (_flags.ekf_bad != AP_my_Notify::flags.ekf_bad) {
        _flags.ekf_bad = AP_my_Notify::flags.ekf_bad;
        if (_flags.ekf_bad) {
    ...
    if (AP_my_Notify::flags.vehicle_lost) {
...
void my_Buzzer::update_playing_pattern()
{
    if (_pattern == 0UL) {
        return;
    }
...
void my_Buzzer::on(bool turn_on)
{
...
void my_Buzzer::play_pattern(const uint32_t pattern)
{
    _pattern = pattern;
