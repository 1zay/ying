...
void my_MMLPlayer::play(const char* string)
{
    stop();
...
void my_MMLPlayer::stop()
{
    _playing = false;
    hal.util->toneAlarm_set_buzzer_tone(0,0,0);
}

void my_MMLPlayer::start_silence(float duration)
{
...
void my_MMLPlayer::start_note(float duration, float frequency, float volume)
{
    _note_start_us = AP_HAL::micros();
    _note_duration_us = duration*1e6;
...
uint8_t my_MMLPlayer::next_number()
{
    uint8_t ret = 0;
...
float my_MMLPlayer::rest_duration(uint32_t rest_length, uint8_t dots)
{
...
void my_MMLPlayer::next_action()
{
    ...
    note_volume *= AP::my_Notify().get_buzz_volume() * 0.01;
