...
my_NeoPixel::my_NeoPixel() :
    my_RGBLed(NEOPIXEL_LED_OFF, NEOPIXEL_LED_HIGH, NEOPIXEL_LED_MEDIUM, NEOPIXEL_LED_LOW)
{
}

bool my_NeoPixel::hw_init()
{
    init_ports();
    hal.scheduler->register_io_process(FUNCTOR_BIND_MEMBER(&my_NeoPixel::timer, void));
    return true;
}

uint16_t my_NeoPixel::init_ports()
{
    uint16_t mask = 0;
...
bool my_NeoPixel::hw_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    if (enable_mask == 0) {
